...
from filter_processing import highpass_filter, bandpass_filter, lowpass_filter, bandstop_filter, gain_control_filter
...
def getPlots(option, filename):
    ...
    if option == 1:
        ...
        axs[0].plot(fft_band_data_breathing_freq * 60, np.abs(fft_band_data_breathing))
        axs[0].axvline(x=best_breathing_freq * 60, color='g', linestyle='dotted', label='Breathing Frequency', linewidth=2)
        axs[0].annotate('Breathing BPM = %.2f' % (best_breathing_freq * 60), xy=(0.20, 0.90), xycoords='axes fraction', color='green', fontsize=10, weight='bold')
        axs[0].set_title('Breathing Region Filtered Data')
        axs[0].set_xlabel('Frequency (BPM)')
        axs[0].set_ylabel('Magnitude')

        axs[1].plot(fft_band_data_cardiac_freq * 60, np.abs(fft_band_data_cardiac))
        axs[1].axvline(x=best_cardiac_freq * 60, color='g', linestyle='dotted', label='Cardiac Frequency', linewidth=2)
        axs[1].annotate('Cardiac BPM = %.2f' % (best_cardiac_freq * 60), xy=(0.20, 0.90), xycoords='axes fraction', color='green', fontsize=10, weight='bold')
        axs[1].set_title('Cardiac Region Filtered Data')
    ...
    if option == 2:
        ...
        axs[1, 0].plot(phase_time[:-1], gain_control_chest_displacement, "b")
...
if __name__ == "__main__":
    getPlots(1,"DCA1000EVM_normalbreathingat1m_12br_82hr.csv")
    getPlots(2,"DCA1000EVM_normalbreathingat1m_12br_82hr.csv")
